...
from giveme5w1h.beneparcomponent import BeneparComponent
from giveme5w1h.preprocessor import PreprocessorSpacy,remove_unserializable_results
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p",dest="n_processes", type=int, default=1, help="Number of processes to use")
    parser.add_argument("-m",dest="nlp_model", type=str, default="en_core_web_sm", help="spaCy model to use, or coreNLP")
    ...
    parser.add_argument("--no_threading",dest="threaded_extraction",action="store_false", help="Disable multithreaded extraction")
    parser.set_defaults(threaded_extraction=True)
    parser.add_argument("--skip_where",action="store_true", help="Skip where extraction")
    parser.set_defaults(skip_where=False)
    parser.add_argument("--errors",dest="skip_errors",action="store_false", help="exit on errors")
    parser.set_defaults(skip_errors=True)
    parser.add_argument("--hide_errors",dest="show_errors",action="store_false", help="hide errors")
    ...
    run_5w1h_extract(n_processes=args.n_processes, nlp_model=args.nlp_model, use_gpu=args.use_gpu, force=args.force,
                     threaded_extraction=args.threaded_extraction,skip_where=args.skip_where,skip_errors=args.skip_errors,show_errors=args.show_errors)