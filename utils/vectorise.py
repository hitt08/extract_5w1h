...
from .tokenize import Tokenizers
...
def tfidf_vectorize(doc_data, max_df=0.95, nltk_dir="/app/nltk_data", lsa=True, lsa_dim=200, split_tokenize=False):
    tokenizer = Tokenizers(nltk_dir=nltk_dir).tokenize_split if split_tokenize else Tokenizers(nltk_dir=nltk_dir).tokenize_pnct_lemma
    vect = TfidfVectorizer(tokenizer=tokenizer, strip_accents='ascii', max_df=max_df, max_features=100000, token_pattern=None)
